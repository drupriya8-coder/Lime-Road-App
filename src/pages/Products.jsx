// src/pages/Products.jsx
import React from "react";
import ProductCard from "../components/ProductCard";
import { Link } from "react-router-dom";

const products = [
    {
        image: "https://images.unsplash.com/photo-1511707171634-5f897ff02aa9", category: "Electronics", name: "Smartphone", price: 15000
    },
    {
        image: "https://rukminim2.flixcart.com/image/704/844/xif0q/t-shirt/b/y/w/s-mens-polyester-white-tshirt-half-sleeves-amsha-original-imahayaxpdgn5uga.jpeg?q=90&crop=false", category: "Clothing", name: "T-Shirt", price: 799
    },
    {
        image: "https://upload.wikimedia.org/wikipedia/commons/thumb/6/68/King_Street_Sneak.JPG/250px-King_Street_Sneak.JPG",
        category: "Footwear",
        name: "Sneakers",
        price: 2500
    },
    {
        image: "https://images.unsplash.com/photo-1523275335684-37898b6baf30",
        category: "Accessories",
        name: "Wrist Watch",
        price: 3000
    },
    {
        image: "https://i.pinimg.com/736x/d7/cd/14/d7cd14b1244b7c86898b9ba6b1283704.jpg",
        category: "Home",
        name: "Mixer Grinder",
        price: 4500
    },
    {
        image: "https://images.unsplash.com/photo-1522336572468-97b06e8ef143",
        category: "Beauty",
        name: "Perfume",
        price: 1200
    },
    {
        image: "https://images.unsplash.com/photo-1512820790803-83ca734da794",
        category: "Books",
        name: "JavaScript Guide",
        price: 499
    },
    {
        image: "https://www.chemicalsafetyfacts.org/wp-content/uploads/shutterstock_383521510-002-scaled.jpg",
        category: "Toys",
        name: "Remote Car",
        price: 2200
    },
    {
        image: "https://images.unsplash.com/photo-1600891964599-f61ba0e24092",
        category: "Kitchen",
        name: "Dinner Set",
        price: 3500
    },
    {
        image: "https://etimg.etb2bimg.com/photo/74881928.cms",
        category: "Sports",
        name: "Cricket Bat",
        price: 1800
    }
];

const Products = () => {
    return (
        <section className="text-gray-600 body-font">
            <div className="container px-0 py-4 mx-auto">
                <div className="flex flex-wrap -m-2 h-[420px] overflow-y-scroll border border-gray-300 rounded-lg shadow-lg p-4">
                    {products.map((item, index) => (
                        <Link key={index} to={`/products/${index + 1}`}>
                            <ProductCard
                                image={item.image}
                                category={item.category}
                                name={item.name}
                                price={item.price}
                            />
                        </Link>
                    ))}

                </div>
            </div>
        </section>
    );
};

export default Products;
